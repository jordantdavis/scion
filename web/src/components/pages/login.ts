...
export class ScionLoginPage extends LitElement {
  ...
  /**
   * Whether the custom (corporate SSO) OAuth provider is configured (fetched from server)
   */
  @state()
  private customEnabled = false;

  /**
   * Display name for the custom OAuth provider (fetched from server)
   */
  @state()
  private customDisplayName = 'SSO';
  ...
  private getProviders(): OAuthProvider[] {
    return [
      ...
      // Corporate SSO's absence should be invisible: only include the button
      // when configured, unlike google/github which always render (disabled).
      ...(this.customEnabled
        ? [
            {
              id: 'custom',
